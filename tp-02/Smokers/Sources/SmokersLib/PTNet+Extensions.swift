...
public func seen(p: PTMarking, q: [MarkingGraph]) -> Bool {
        // Cette fonction prend en paramètre un marquage "p" et une liste de
        // marquages "q". Si jamais le marquage se trouve dans la liste de
        // marquages, la fonction retourne 'false' pour dire que ce marquage n'est
        // pas nouveau. Sinon elle retourne'true' pour dire qu'elle ne l'a jamais vu.
    for n in q {
        if(n.marking == p){
            return false
                // pour chaque marquage "n" dans la liste de marquages
