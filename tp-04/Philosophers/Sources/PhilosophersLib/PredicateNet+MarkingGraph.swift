extension PredicateNet {
    ...
    public func markingGraph(from marking: MarkingType) -> PredicateMarkingNode<T>? {
        ...
        let m0 = PredicateMarkingNode<T>(marking: marking, successors: [:])
        var visited = [m0]
        var toVisit = [m0]

         while let presentMarking = toVisit.popLast(){
             for presentTransition in self.transitions{
                 presentMarking.successors[presentTransition] = [:]
                 let presentBinding = presentTransition.fireableBingings(from: presentMarking.marking)
                 for bindings in presentBinding {
                     let nextMarking = presentTransition.fire(from: presentMarking.marking, with: bindings)
                     if (nextMarking != nil) {
                         if (visited.contains(where: {PredicateNet.greater(nextMarking!, $0.marking)})) {
                             return nil
                         }
                         if !(visited.contains(where: {PredicateNet.equals($0.marking, nextMarking!)})) {
                             let newMark = PredicateMarkingNode<T>(marking: nextMarking!, successors: [:])
                             presentMarking.successors[presentTransition]![bindings] = newMark
                             toVisit.append(newMark)
                             visited.append(newMark)
                         }
                         else {
                             let mark = visited.index(where : {PredicateNet.equals($0.marking, nextMarking!)})
                         presentMarking.successors[presentTransition]![bindings] = visited[mark!]
                         }
                     }
                 }
             }
